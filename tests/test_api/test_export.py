...
async def test_export_ca_certificate(client: AsyncClient):
    ...
    assert response.headers["Content-Disposition"] == f"attachment; filename=ca_{ca_id}_certificate.pem"
...
async def test_export_ca_private_key(client: AsyncClient):
    ...
    assert response.status_code == status.HTTP_200_OK
    assert response.headers["Content-Type"] == "application/x-pem-file"
    assert response.headers["Content-Disposition"] == f"attachment; filename=ca_{ca_id}_private_key.pem"
...
async def test_export_certificate(client: AsyncClient):
    ...
    assert response.status_code == status.HTTP_200_OK
    assert response.headers["Content-Type"] == "application/x-pem-file"
    assert response.headers["Content-Disposition"] == f"attachment; filename=certificate_{cert_id}.pem"
    assert "-----BEGIN CERTIFICATE-----" in response.text
    assert "-----END CERTIFICATE-----" in response.text
...
async def test_export_certificate_private_key(client: AsyncClient):
    ...
    response = await client.get(f"{settings.API_V1_STR}/export/certificate/{cert_id}/private-key")
    
    assert response.status_code == status.HTTP_200_OK
    assert response.headers["Content-Type"] == "application/x-pem-file"
    assert response.headers["Content-Disposition"] == f"attachment; filename=certificate_{cert_id}_private_key.pem"
    assert "-----BEGIN PRIVATE KEY-----" in response.text
    assert "-----END PRIVATE KEY-----" in response.text
...
async def test_export_certificate_chain(client: AsyncClient):
    ...
    response = await client.get(f"{settings.API_V1_STR}/export/certificate/{cert_id}/chain")
    
    assert response.status_code == status.HTTP_200_OK
    assert response.headers["Content-Type"] == "application/x-pem-file"
    assert response.headers["Content-Disposition"] == f"attachment; filename=certificate_{cert_id}_chain.pem"
