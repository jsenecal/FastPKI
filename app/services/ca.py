...
class CAService:
    ...
    def generate_key_pair(key_size: int = 2048) -> Tuple[rsa.RSAPrivateKey, bytes]:
        """Generate an RSA key pair and return the key object and PEM-encoded private key."""
    ...
    def parse_subject_dn(subject_dn: str) -> x509.Name:
        ...
        if "O" in parts:
            name_attributes.append(x509.NameAttribute(NameOID.ORGANIZATION_NAME, parts["O"]))
        if "OU" in parts:
            name_attributes.append(x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, parts["OU"]))
        if "C" in parts:
            name_attributes.append(x509.NameAttribute(NameOID.COUNTRY_NAME, parts["C"]))
        if "ST" in parts:
            name_attributes.append(x509.NameAttribute(NameOID.STATE_OR_PROVINCE_NAME, parts["ST"]))
        if "L" in parts:
            name_attributes.append(x509.NameAttribute(NameOID.LOCALITY_NAME, parts["L"]))
